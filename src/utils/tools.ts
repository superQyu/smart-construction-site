import type { MenuItem, TreeNode, TreeParam } from '@/types/router.type';
/**
...
export const url2json = (url: string = '?') => {
    if (!url) url = location.search;
    const [u, p = ""] = url.split("?");
    return [u, p.split("&").reduce((acc: Record<string, string>, cur) => {
 ...
 * @return {Array}
 */
export const buildTree = (arr: TreeNode[], params?: any) => {
    ...
    for (const node of arr) {
        ...
        if (defaultParams.intercept && typeof defaultParams.intercept === 'function') curItem = defaultParams.intercept(curItem);
...
export const sortMenu = (arr: TreeNode[], sortByKey = "orderNum"): MenuItem[] => {
    ...
    return sortedArr.sort((a, b) => a[sortByKey] - b[sortByKey]).map((menu) => {
