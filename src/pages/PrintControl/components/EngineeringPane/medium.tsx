...
import MediumDetail from './details/medium';
...
export default function MediumColumn(props: Props) {
  ...
  const [dialogType, setDialogType] = useState<DialogType[]>([
    {
      key: 'detail',
      label: t('printControl.setting.mediumDetails', '介质详细信息'),
      compoment: <MediumDetail params={mediumItem} />,
    },
    {
      key: 'add',
      label: t('printControl.engineeringPane.addMedium', '添加介质'),
      compoment: <AddMedium />,
    },
  ]);
  ...
  useEffect(() => {
    setMediumColumnList([
      t('mdium.setting.medisName1', '介质1'),
      t('mdium.setting.medisName2', '介质2'),
      t('mdium.setting.medisName3', '介质3'),
    ]);
      ...
      <Box className="h-full" sx={{ padding: '1px' }}>
        ...
        <Box className="flex " sx={defSX.buttonBox}>
          <CusButton onClick={() => handlerItem(null, -1, 1)}>
            {t('genericName.loadingMedium', '添加介质')}
          </CusButton>
          <CusButton sx={{ backgroundColor: '#dfe0e3', color: '#4b4b4d' }}>
            {t('genericName.deleteMedium', '删除介质')}
          </CusButton>
        </Box>
      </Box>
      <CustomDialog
